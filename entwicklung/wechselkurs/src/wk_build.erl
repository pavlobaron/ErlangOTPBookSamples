...
files() -> 
    ["wechselkurs_server.erl" ,
     ...
     "../test/wechselkurs_tests.erl",  
     "../test/wechselkurs_more_tests.erl",
     "wechselkurs_db.erl",
     "wechselkurs_app.erl"].
    
compile() ->
    io:format("COMPILE MODULES ...\n"),
    lists:foreach(fun(File) -> c:c(File, [debug_info, {i, "../include"}, {outdir, "../ebin"}]), io:format("Module ~p compiled \n", [File]) end, files()),
    io:format("...\n\n\n").
...
doc() ->
    ...
    io:format("result: ~p~n", [edoc:application(wechselkurs, "../", [{todo, true}, {includes, "../include"}])]),
    io:format("...\n\n\n").

perf_kursinfo1() -> 
    io:format("FPROF ...\n"),
    wechselkurs_server:start(),
    fprof:trace(start, "../ftrace.trace"),
...
cover() ->
    io:format("COVER ... \n"),
    cover:start(),
    cover:compile(wechselkurs_server, [{i, "../include"}]),
    wechselkurs_server:start(),
    [ wechselkurs_client:kursinfo() || _P <- lists:seq(1,10)],
    [ wechselkurs_client:umrechnung() || _P <- lists:seq(1,10)],
    cover:analyse_to_file(wechselkurs_server, "../cover.out"),
    wechselkurs_server:stop(),
    cover:stop(),
    io:format("...\n\n\n").
